...
class PolynomialFeaturesPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        test_aligned = test_x.reindex(columns=train_x.columns, fill_value=0)
        test_data_x = self.scaler.transform(test_aligned)
...
class InteractionFeaturesPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        train_data_x = train_data_x.loc[:, ~train_data_x.columns.duplicated()]
        # 处理数值溢出和无穷大值
        train_data_x = train_data_x.replace([np.inf, -np.inf], np.nan)
        # 对过大的值进行截断，防止float64溢出
        max_value = 1e308  # float64的最大安全值
        train_data_x = train_data_x.clip(-max_value, max_value)
        ...
        test_data_x = test_data_x.loc[:, ~test_data_x.columns.duplicated()]
        # 对测试数据应用相同的处理
        test_data_x = test_data_x.replace([np.inf, -np.inf], np.nan)
        test_data_x = test_data_x.clip(-max_value, max_value)
...
class PCA_AUTO_Prim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        test_aligned = test_x.reindex(columns=cols, fill_value=0)
        test_data_x = self.pca.transform(test_aligned)
...
class RandomTreesEmbeddingPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        new_cols = [str(i) for i in range(train_data_x.shape[1])]
        train_data_x = pd.DataFrame(train_data_x, columns=new_cols)

        test_data_x = self.pca.transform(test_aligned).toarray()
        new_cols = [str(i) for i in range(test_data_x.shape[1])]
