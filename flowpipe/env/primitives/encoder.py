...
class LabelEncoderPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        # 1) 合并类别列，一次性编码
        combined = pd.concat([cat_trainX, cat_testX], axis=0, ignore_index=True)

        for col in cols:
            encoder = LabelEncoder()
            combined[col] = encoder.fit_transform(combined[col].astype(str))
            self.preprocess[col] = encoder

        # 2) 按原长度拆回训练/测试
        cat_trainX = combined.iloc[: len(cat_trainX)].reset_index(drop=True)
        cat_testX = combined.iloc[len(cat_trainX):].reset_index(drop=True)

        # 3) 拼回数值列
        train_data_x = pd.concat(
            [cat_trainX, num_trainX.reset_index(drop=True)],
            axis=1,
        ).infer_objects()
        test_data_x = pd.concat(
            [cat_testX, num_testX.reset_index(drop=True)],
            axis=1,
        ).infer_objects()
