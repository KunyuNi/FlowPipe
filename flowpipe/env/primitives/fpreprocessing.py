...
class MinMaxScalerPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        # 拆成数值列和类别列
        cat_train, num_train = catch_num(train_x)
        cat_test, num_test = catch_num(test_x)

        # 记录原始索引，供最后拆回
        n_train = len(cat_train)
        n_test = len(cat_test)

        # 1) 合并类别列，统一处理（如果类别本身需要编码，可以在这里做；当前只是合并后原样拆回）
        combined_cat = pd.concat([cat_train, cat_test], axis=0, ignore_index=True)

        # 2) 合并数值列，统一做缩放
        combined_num = pd.concat([num_train, num_test], axis=0, ignore_index=True)
        self.scaler.fit(combined_num)
        combined_num_scaled = pd.DataFrame(
            self.scaler.transform(combined_num),
            columns=combined_num.columns,
        )

        # 3) 按原长度拆回 train/test
        cat_train_part = combined_cat.iloc[:n_train].reset_index(drop=True)
        cat_test_part = combined_cat.iloc[n_train:].reset_index(drop=True)

        num_train_part = combined_num_scaled.iloc[:n_train].reset_index(drop=True)
        num_test_part = combined_num_scaled.iloc[n_train:].reset_index(drop=True)

        # 4) 拼回 train/test
        train_data_x = pd.concat([cat_train_part, num_train_part], axis=1)
        test_data_x = pd.concat([cat_test_part, num_test_part], axis=1)
...
class NormalizerPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        num_train_scaled = transformed_num.iloc[: len(num_train)].reset_index(drop=True)
        num_test_scaled = transformed_num.iloc[len(num_train):].reset_index(drop=True)

        train_data_x = pd.concat(
            [cat_train.reset_index(drop=True), num_train_scaled], axis=1
        )
        test_data_x = pd.concat(
            [cat_test.reset_index(drop=True), num_test_scaled], axis=1
        )
        return train_data_x, test_data_x
...
class KBinsDiscretizerOrdinalPrim(Primitive):
    ...
    def transform(self, train_x, test_x, train_y):
        ...
        train_cat_cols = list(cat_train_x.columns)
        train_num_cols = list(num_train_x.columns)

        # 将测试集对齐到训练集的列（缺的补 0）
        cat_test_x = cat_test_x.reindex(columns=train_cat_cols, fill_value=0)
        num_test_x = num_test_x.reindex(columns=train_num_cols, fill_value=0)

        # 改进：确保数值列名唯一
        if len(train_num_cols) != len(set(train_num_cols)):
            # 如果存在重复列名，需要去重
            seen = set()
            unique_num_cols = []
            for col in train_num_cols:
                if col not in seen:
                    ...
                    while new_col in seen:
                        ...
                        new_col = f"{col}_{counter}"
                    seen.add(new_col)
                    unique_num_cols.append(new_col)
                    # 重命名列
                    num_train_x = num_train_x.rename(columns={col: new_col})
                    num_test_x = num_test_x.rename(columns={col: new_col})
            train_num_cols = unique_num_cols

        # 建立并拟合 ColumnTransformer
        self.scaler = ColumnTransformer(
            [("discrit", KBinsDiscretizer(encode="ordinal"), train_num_cols)]
        )
        self.scaler.fit(num_train_x)

        num_train_x = (
            pd.DataFrame(
                self.scaler.transform(num_train_x), columns=train_num_cols
            )
        ...
        num_test_x = (
            pd.DataFrame(
                self.scaler.transform(num_test_x), columns=train_num_cols
            )
            .reset_index(drop=True)
            .infer_objects()
        )
        ...
        train_data_x = pd.concat(
            [cat_train_x.reset_index(drop=True), num_train_x], axis=1
        )
        test_data_x = pd.concat(
            [cat_test_x.reset_index(drop=True), num_test_x], axis=1
        )
        return train_data_x, test_data_x
