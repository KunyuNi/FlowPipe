...
from ..ctx import TableEmbedder, TextEmbedder, embedder  # 调用预训练模型
...
class LogicalPipelineModel(AgentModel): # 初始化 普通DQN 用于从预定义的 pipeline 顺序中选择合适的
...
class Agent:
    ...
    def act(
        ...
        if self.no_random or (randnum > epsilon):
            ...
            # 统一使用 trainer 注入的 4096 维上下文
            if self.last_raw_dataset_ctx is None:
                raise RuntimeError("last_raw_dataset_ctx is None. Ensure trainer sets a 4096-dim ctx before act().")
            ctx_embeddings = self.last_raw_dataset_ctx
            if not isinstance(ctx_embeddings, torch.Tensor):
                ctx_embeddings = torch.tensor(ctx_embeddings, dtype=torch.float)

            if ctx_embeddings.dim() == 1:
                ctx_embeddings = ctx_embeddings.unsqueeze(0)  # (1, 4096)
    ...
    def learn_components(self, test=False) -> None:
        ...
        s1_imputernum = util.device_tensor(s1_imputernum)
